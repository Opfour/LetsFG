"""
SriLankan Airlines (UL) -- EveryMundo sputnik fare search API connector.

SriLankan Airlines (IATA: UL) is the flag carrier of Sri Lanka.
Hub at Colombo Bandaranaike (CMB) with routes to India, Middle East,
...
Strategy (direct API — no browser required):
  ...
  3. Construct booking URL for srilankan.com
"""
...
_SPUTNIK_URL = (
    ...
    "/airfare-sputnik-service/v3/ul/fares/search"
)
...
_HOME_URL = "https://www.srilankan.com/en_uk/"


class SrilankanConnectorClient:
    """SriLankan Airlines (UL) — EveryMundo sputnik fare search API."""
    ...
    async def _search_ow(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        logger.info(
            "SriLankan Airlines %s→%s: %d offers in %.1fs",
            req.origin, req.destination, len(offers), elapsed,
        )

        h = hashlib.md5(
            f"ul{req.origin}{req.destination}{req.date_from}{req.return_from}".encode()
        ).hexdigest()[:12]
    ...
    async def _call_sputnik(self, payload: dict) -> list[dict]:
        try:
            ...
            if r.status_code != 200:
                logger.warning(
                    "SriLankan Airlines sputnik: %d %s",
            ...
            logger.error("SriLankan Airlines sputnik error: %s", e)
            return []
    ...
    def _build_offer(
        ...
        offer_hash = hashlib.md5(
            f"ul_{origin_code}_{dest_code}_{dep_date_str}_{ret_date_str}_{price_f}".encode()
        ).hexdigest()[:12]
        ...
        return FlightOffer(
            ...
            conditions={
                ...
                "fare_note": "Fare from SriLankan Airlines EveryMundo module",
            },
        )
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_sril_{cid}", price=price, currency=o.currency,
