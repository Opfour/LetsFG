...
class FinnairConnectorClient:
    ...
    async def _fetch_ancillaries(
        ...
        try:
            ...
            result = await probe_ancillaries("AY", origin, dest, date_str=date_str)
            if result:
                return result
        except Exception:
            pass
        # Static fallback: Finnair Economy Light: no bag. Economy Classic/Flex: 1×23 kg included.
        return {
            "bags_note": "Economy Light (cheapest): no checked bag, first bag from EUR 25. Economy Classic/Flex: 1×23 kg included. Carry-on 8 kg included.",
            "seat_note": "Seat selection: from EUR 15 (Light). Standard seat included on Classic/Flex fares.",
            "bags_from": None,
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_finn_{cid}", price=price, currency=o.currency,
