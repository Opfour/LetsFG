...
_USER_DATA_DIR = os.path.join(
    os.environ.get("TEMP", os.environ.get("TMPDIR", "/tmp")), ".av_chrome_data"
)
...
class AviancaConnectorClient:
    ...
    def _extract_av_bag_info(ab: dict, currency: str) -> tuple[dict, dict]:
        ...
        bags_price.setdefault("seat", 10.0)  # seat selection from ~USD 10 on Basic fare
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_av_{cid}", price=price, currency=o.currency,
