"""
BYOjet connector — Australian OTA (Flight Centre group, CDP Chrome).

BYOjet is part of Flight Centre Travel Group. Shows consolidator fares
especially strong on AU/NZ/Asia routes. Also operates under Aunt Betty brand
(same backend).

Strategy (CDP Chrome + deep-link + API interception):
1. Launch real Chrome via --remote-debugging-port.
2. Navigate directly to search results URL (deep-link, no form fill needed).
3. Intercept JSON API responses with flight data.
4. Also try extracting __NEXT_DATA__ / window globals as fallback.
"""
...
async def _get_browser():
    ...
    async with lock:
        ...
        _pw_instance = pw
        _browser = await pw.chromium.connect_over_cdp(f"http://127.0.0.1:{_CDP_PORT}")
        logger.info("BYOjet: Chrome launched on CDP port %d (pid %d)", _CDP_PORT, _chrome_proc.pid)
        return _browser


async def _dismiss_cookies(page) -> None:
    """Dismiss OneTrust cookie banner on BYOjet."""
    try:
        btn = page.locator('#onetrust-accept-btn-handler')
        if await btn.count() > 0:
            await btn.click(timeout=3000)
            await asyncio.sleep(0.5)
            logger.info("BYOjet: dismissed OneTrust cookie banner")
            return
    except Exception:
        pass
...
class ByojetConnectorClient:
    """BYOjet — Australian OTA (Flight Centre group), CDP Chrome + deep-link."""
    ...
    async def _search_ow(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            # Build deep-link URL to booking.byojet.com
            ...
            logger.info("BYOjet %s→%s: %d offers in %.1fs (CDP Chrome)", req.origin, req.destination, len(offers), elapsed)

            sh = hashlib.md5(f"byojet{req.origin}{req.destination}{date_str}".encode()).hexdigest()[:12]
            ...
            logger.error("BYOjet CDP error: %s", e)
    ...
    # Airline code → name map (common Australian carriers)
    _AIRLINE_NAMES = {
        "VA": "Virgin Australia", "QF": "Qantas Airways", "JQ": "Jetstar Airways",
        "ZL": "Regional Express", "TT": "Tigerair Australia", "NZ": "Air New Zealand",
    ...
    def _parse(self, data: dict, req: FlightSearchRequest, date_str: str) -> list[FlightOffer]:
        """Parse BYOjet Api/search response."""
        offers: list[FlightOffer] = []
        ...
        # Get currency from search info
        search_info = data.get("search") or {}
        currency = "AUD"  # BYOjet prices in AUD
        ...
        for item in results[:50]:
            try:
                ...
                # Use first journey (outbound)
                journey = journeys[0]
                journey_segs = journey.get("segments") or []
                if not journey_segs:
                    continue

                # Journey-level airports
                ...
                for si, seg in enumerate(journey_segs):
                    ...
                    # Segments don't have explicit airport codes;
                    # use journey origin/dest for first/last, infer for middle
                    ...
                    segments.append(FlightSegment(
                        airline=airline_name or "BYOjet",
                ...
                if total_dur:
                    total_dur_secs = int(total_dur) * 60
                else:
                    total_dur_secs = sum(s.duration_seconds for s in segments)

                route = FlightRoute(
                    segments=segments, total_duration_seconds=total_dur_secs,
                    stopovers=max(0, int(j_stops)),
                )
                oid = hashlib.md5(f"byoj_{req.origin}{req.destination}{date_str}{price}{segments[0].flight_no}".encode()).hexdigest()[:12]

                offers.append(FlightOffer(
                    id=f"byoj_{oid}", price=round(price, 2), currency=currency,
                    price_formatted=f"{price:.2f} {currency}",
                    outbound=route, inbound=None,
                    airlines=list({s.airline for s in segments if s.airline}),
                    owner_airline=segments[0].airline if segments else "BYOjet",
                    booking_url=f"https://booking.byojet.com/#/au/search?OriginCode={req.origin}&DestinationCode={req.destination}",
                    is_locked=False, source="byojet_ota", source_tier="free",
                ))
            except Exception as e:
                logger.debug("BYOjet parse error: %s", e)
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                combos.append(FlightOffer(
                    id=f"rt_byoj_{cid}", price=price, currency=o.currency,
    ...
    def _empty(self, req: FlightSearchRequest) -> FlightSearchResponse:
        h = hashlib.md5(f"byojet{req.origin}{req.destination}{req.date_from}{req.return_from or ''}".encode()).hexdigest()[:12]
        return FlightSearchResponse(
    ...
    async def _fetch_ancillaries(
        ...
        return {
            ...
            "seat_note": "seat selection varies; skip at checkout for free random seat",
        }
