"""
Air France (AF) -- EveryMundo sputnik grouped-routes API connector.

Uses the airTRFX fare-finder API to retrieve published fares across Air France's network.
Markets: US, FR, NL, GB, DE.
"""
...
_HOME_URL = "https://www.airfrance.fr/en"
_API_URL = "https://openair-california.airtrfx.com/airfare-sputnik-service/v3/af/fares/grouped-routes"
...
class AirfranceConnectorClient:
    ...
    async def _fetch_ancillaries(
        ...
        try:
            ...
            result = await probe_ancillaries("AF", origin, dest, date_str=date_str)
            if result:
                return result
        except Exception:
            pass
        # Static fallback: Air France Light fare: no bag; Standard: 1×23 kg included.
        return {
            "bags_note": "Economy Light: no checked bag, first bag from EUR 20. Economy Standard/Flex: 1×23 kg included. Carry-on included on all fares.",
            "seat_note": "Seat selection: from EUR 15 (Light). Standard seat included on Standard/Flex fares.",
            "bags_from": None,
            "checked_bag_price": 20.0,
            "currency": currency,
        }
    ...
    async def _search_ow(self, req):
        ...
        search_hash = hashlib.md5(
            f"airfrance{req.origin}{req.destination}{req.date_from}{req.return_from}".encode()
        ).hexdigest()[:12]
