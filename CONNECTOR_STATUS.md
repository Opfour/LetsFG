...
|---|-----------|------|-------|--------|------------|-----------|-----------|
| 1 | airasia | AK | [#19](https://github.com/Boosted-Chat/LetsFG/issues/19) | `done` | copilot-main | 2026-03-13T17:00Z | headed Chrome + API interception |
| 2 | azul | AD | [#33](https://github.com/Boosted-Chat/LetsFG/issues/33) | `done` | claude-connector-fix | 2026-03-13T21:00Z | Switched to headed Chrome + route interception. Akamai blocks headless; SPA sends empty criteria → rewrite via page.route(). Tested VCP→CNF (8), GRU→SSA (20), CNF→SSA (26). |
| 3 | batikair | ID | — | `done` | copilot-batikair-fix | 2026-03-13T18:11Z | nodriver CF bypass + PW DOM extraction, MYR |
| 4 | cebupacific | 5J | [#16](https://github.com/Boosted-Chat/LetsFG/issues/16) | `done` | copilot-main | 2026-03-13T18:15Z | MCP-style Chrome flags bypass Akamai; SOAR API interception |
| 5 | easyjet | U2 | [#20](https://github.com/Boosted-Chat/LetsFG/issues/20) | `done` | copilot-batikair-fix | 2026-03-13T21:11Z | Headed Chrome + form fill + response interception. Akamai blocks headless; fresh profile on 403. Tested LGW→BCN (4 offers). |
...
| 8 | gol | G3 | [#34](https://github.com/Boosted-Chat/LetsFG/issues/34) | `done` | claude-connector-fix | 2026-03-13T21:07Z | Switched to headed Chrome. Akamai blocked headless; UUID now populates via waitForFunction. Tested GRU->GIG (5), CGH->SDU (24). |
| 9 | indigo | 6E | [#17](https://github.com/Boosted-Chat/LetsFG/issues/17) | `done` | claude-connector-fix | 2026-03-13T21:08Z | headed Chrome + city-selection selectors (73 offers DEL→BOM) |
| 10 | jet2 | LS | [#32](https://github.com/Boosted-Chat/LetsFG/issues/32) | `done` | copilot-eurowings-fix | 2026-03-13T21:30Z | persistent headed Chrome for Akamai bypass |
...
| 12 | jetstar | JQ | [#31](https://github.com/Boosted-Chat/LetsFG/issues/31) | `done` | copilot-eurowings-fix | 2026-03-14T00:00Z | CDP Chrome + Kasada warm-up bypass |
| 13 | lionair | JT | [#35](https://github.com/Boosted-Chat/LetsFG/issues/35) | `removed` | copilot-main | 2026-03-14T12:00Z | booking.lionair.co.id connection refused (IBE2 dead), connector removed |
...
| 16 | norwegian | DY | [#22](https://github.com/Boosted-Chat/LetsFG/issues/22) | `done` | copilot-batikair-fix | 2026-03-13T21:17Z | Headed CDP Chrome for Incapsula cookies; 20 offers OSL→LGW 1.1s cached |
| 17 | peach | MM | [#36](https://github.com/Boosted-Chat/LetsFG/issues/36) | `done` | claude-connector-fix | 2026-03-13T21:42Z | Headed CDP Chrome (no headless) + disable-http2 + modal dismissal; reCAPTCHA auto-passes with real CDP Chrome |
| 18 | pegasus | PC | [#37](https://github.com/Boosted-Chat/LetsFG/issues/37) | `done` | copilot-main | 2026-03-13T22:15Z | MCP-style Chrome flags bypass Akamai; direct booking URL + /pegasus/availability interception; 40 offers SAW→AYT |
| 19 | porter | — | [#24](https://github.com/Boosted-Chat/LetsFG/issues/24) | `done` | copilot-eurowings-fix | 2026-03-14T01:30Z | MCP Chrome flags bypass Cloudflare; direct URL + DOM scrape; 6 flights 12 offers YTZ→YOW |
| 20 | scoot | TR | [#30](https://github.com/Boosted-Chat/LetsFG/issues/30) | `done` | copilot-batikair-fix | 2026-03-14T00:30Z | Headed CDP Chrome for Akamai bypass; 6 offers SIN→BKK 167.63 SGD |
| 21 | volotea | V7 | [#18](https://github.com/Boosted-Chat/LetsFG/issues/18) | `done` | copilot-batikair-fix | 2026-03-14T01:00Z | Headed CDP Chrome (Incapsula default-ctx bypass); fixed schedule JSON direction bug — reverse file has current data; null field guards |
| 22 | volaris | Y4 | [#21](https://github.com/Boosted-Chat/LetsFG/issues/21) | `done` | copilot-eurowings-fix | 2026-03-14T02:00Z | headed Chrome + es-mx locale + v3 availability parser |
...
|---|-----------|------|-------|--------|--------|
| 26 | allegiant | G4 | [#38](https://github.com/Boosted-Chat/LetsFG/issues/38) | `done` | **US IP required.** Headed Chrome + GraphQL interception. Set ALLEGIANT_PROXY if outside US. |
| 27 | southwest | WN | [#26](https://github.com/Boosted-Chat/LetsFG/issues/26) | `done` | **US IP required.** Headed Chrome + Playwright form fill + API interception. Set SOUTHWEST_PROXY if outside US. |
| 28 | spirit | NK | [#28](https://github.com/Boosted-Chat/LetsFG/issues/28) | `blocked` | PX Enterprise detects all automation (Playwright, patchright, stealth patches) even with US proxy. Token endpoint 403 → Angular app can't search. Tried: proxy+stealth, patchright Chromium, form fill+interception, direct API (Akamai WAF blocks). Code has proxy+patchright+stealth ready for when detection is bypassed (e.g. nodriver/residential proxy). — `copilot-eurowings-fix` |
| 29 | smartwings | QS | [#23](https://github.com/Boosted-Chat/LetsFG/issues/23) | `done` | Headed CDP Chrome + CF Turnstile bypass (launch with URL before CDP attaches) |
| 30 | transavia | HV | [#25](https://github.com/Boosted-Chat/LetsFG/issues/25) | `done` | Headed CDP Chrome to bypass Cloudflare WAF |
| 31 | wizzair | W6 | [#27](https://github.com/Boosted-Chat/LetsFG/issues/27) | `done` | Fixed: launch Chrome headed (Kasada detects --headless=new) — `copilot-eurowings-fix` |
...
| 35 | twayair | TW | [#29](https://github.com/Boosted-Chat/LetsFG/issues/29) | `done` | copilot-batikair-fix | 2026-03-14T04:00Z | Headed CDP Chrome for Akamai bypass; removed nodriver tier; default context cookie persistence; 1 offer GMP→CJU 50600 KRW, 1 offer ICN→NRT 17357 JPY |
