...
│   │   │   └── connectors/          # 75 LCC airline scrapers + infrastructure
│   │   │       ├── __init__.py
...
2. **Local search** — Fires 75 LCC connectors on the user's machine via Playwright + httpx. No API key needed. Both modes run simultaneously and results are merged.

### 75 local LCC Connectors
The `connectors/` directory contains scrapers for 75 low-cost airlines. Three connector patterns:
